...
def build_torque_controller(CP, CP_SP, CI, dt):
  ...
  controlsd_ext runs this only while it's the active steering channel (the carcontroller's torque_active,
  fed back via carStateSP) and keeps it reset otherwise, so its PID never runs open-loop.
  """
